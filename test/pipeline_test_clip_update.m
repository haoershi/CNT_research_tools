clear
cd /Users/meow/Downloads/UPenn/00_Thesis/script

%% Path parameters
ieeg_folder = '/Users/meow/Downloads/UPenn/00_Thesis/script/matlab/ieeg-matlab-1.14.49';
ieeg_pw_file = '/Users/meow/Downloads/UPenn/00_Thesis/script/qaq_ieeglogin.bin';
...
%% Clip parameters
% any name and time errors?
file_name = 'HUP077_phaseII';
times = [1 15]; 
...
name = ptnameC{1}; % any feedback needed?

%% Download data from ieeg.org
data = download_ieeg_data(file_name, ieeg_login, ieeg_pw_file, times, 1);
oldLabels = data.chLabels; % normally has chan loc or not?
old_values = data.values;
fs = data.fs;
nchs = size(old_values,2);

%% Decompose labels
[labels,~,~] = decompose_labels(oldLabels,name); % show changed label?

% for test
label_diff_table = cell2table([setdiff(oldLabels(:,1),labels),setdiff(labels,oldLabels(:,1))], "VariableNames",["Before" "After"]);
label_diff_table.Before = categorical(label_diff_table.Before);
label_diff_table.After = categorical(label_diff_table.After);
if size(label_diff_table,1)>0
    disp('Example changed labels are:');
    disp(label_diff_table(1:min(10,size(label_diff_table,1)),:));
end
...
% for test
disp('Identified extra cranial electrodes:')
disp(labels(extra_cranial))
...
disp('Before whitening')
show_eeg(show_values,fs,show_labels,5)
disp('After whitening')
show_eeg(whitened_values,fs,show_labels,5)
%% Plot the EEG

...
class iEEGPreprocess:
    ...
    def _load_user(self, username):
        ...
        with open(os.path.join(settings.USER_DIR, username[:3]+'_config.json'), 'r') as f:
    ...
    def login_config(self):
        ...
        """
        from CNTtools.tools.login_config import login_config
    ...
    def login(self, username:str = None):
        ...
        if username is not None:
            if username not in self.users:
                raise Exception("CNTtools:InvalidUsername, Please retry or set up iEEG login info using session.login_config() before use! \n")
            else:
    ...
    def _add_data_instance(self,data):
        ...
        add_data = {'filename':data.filename,
                    'start':data.start,
                    'stop':data.stop,
                    'dura':data.dura,
                    'fs':data.fs}
    ...
    def load_data(self,dir,replace = False,default_folder:bool = True):
        ...
        if os.path.isdir(dir):
            ...
            filelist = [dir]
        assert any(file.endswith(".pkl") for file in filelist), "CNTtools:invalidFileFormat"
        for f in filelist:
            f = os.path.join(dir,f)
            if f.endswith(".pkl"):
                data = self._pickle_open(f)
                assert (isinstance(data, iEEGData) or isinstance(data, iEEGPreprocess)), "CNTtools:invalidFileContents"
                if replace:
                    self.datasets = {} 
                    self.meta = pd.DataFrame(columns = ["filename", "start", "stop","select_elecs","ignore_elecs"])
                    self.num_data = self.meta.shape[0]
    ...
    def download_data(self, filename: str, start: Number, stop: Number, 
        ...
        if not hasattr(self, 'user'):
            self.login(username = username) # ensure login, will raise error if user not config
...
class iEEGData:
    ...
    def _download(self,user):
        self.data,self.fs,self.ch_names = tools.get_ieeg_data(user['usr'],user['pwd'],
                                                              self.filename,self.start,
                                                              self.stop,self.select_elecs,self.ignore_elecs)
    ...
    def filter(self, low_freq: Number = 1, high_freq: Number = 120, notch_freq: Number = 60):
        """
    ...
    def cross_corr(self,win = True, win_size = 2):
        ...
        """
        self.conn['cross_corr'],_ = tools.cross_correlation(self.data,self.fs,win = win,win_size = win_size)
    ...
    def coherence(self,win = True, win_size = 2, segment = 1, overlap = 0.5):
        ...
        """
        self.conn['coh'] = tools.coherence(self.data,self.fs,win = win,win_size = win_size,

...
@beartype
def cross_correlation(values: np.ndarray, fs: Number, win:bool = False, win_size:Number = 2, max_lag: Number = 200):
    """
        ...
        for t in range(nw):
            ...
            for col1 in range(nchan):
                for col2 in range(col1, nchan):
                    ...
                    r = correlate(values[window_start[t]:window_start[t] + iw, col1], values[window_start[t]:window_start[t] + iw, col2])
                    r_all[:,n] = r
                    r_all[:,n_rev] = r
